    ...
    template: `
        ...
        <div
            #tooltip
            class="chart-tooltip"
            style="opacity:0; position: absolute; background-color: white; border: 1px solid #ccc; padding: 10px; pointer-events: none;"
        ></div>
        </div>
    `,
    ...
    private svg: any;
    private tooltip: any;
    private margin = { top: 60, right: 20, bottom: 80, left: 70 };
    ...
    createChart(): void {
        ...
        .attr('x', (d: BarChartDataPoint) =>
            d?.[this.categoryField] ? x(d[this.categoryField]) : 0
        )
        .attr('y', (d: BarChartDataPoint) => y(d[this.valueField]))
        .attr('width', x.bandwidth())
        .attr(
            'height',
            (d: BarChartDataPoint) => this.height - y(d[this.valueField])
        )
        .attr('fill', '#f9e3b7')
        .attr('stroke', '#b16c34')
        .attr('stroke-width', 1)
        .on('mouseover', (event: any, d: BarChartDataPoint) => {
            ...
            this.tooltip
            .html(`${d[this.categoryField]}: ${d[this.valueField].toFixed(2)} €`) // Formatea el valor a 2 decimales
